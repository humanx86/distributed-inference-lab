// Original teaching harness. CUDA runtime validation is required on your GPU.
...
struct Buffer {
    ...
    explicit Buffer(size_t n) {
        CUDA_OK(cudaMalloc(reinterpret_cast<void**>(&p),
                           std::max(size_t(1), n) * sizeof(float)));
    }
...
__global__ void elementwise(const float* x, const float* y, float* z,
                            size_t n, int op) {
...
struct Options {
    ...
    std::string fixture = "normal";  // internal correctness-only fixture
    size_t n = 1 << 20;
...
Options parse(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (k == "--help") {
            std::cout
                << "--op copy|add|transform|reduce --variant pair|tree|warp|cub\n"
    ...
    if (o.op != "copy" && o.op != "add" &&
        o.op != "transform" && o.op != "reduce")
        throw std::runtime_error("Invalid op");

    if (o.variant != "pair" && o.variant != "tree" &&
        o.variant != "warp" && o.variant != "cub")
        throw std::runtime_error("Invalid variant");

    if (o.dataset != "dyadic" && o.dataset != "random" &&
        o.dataset != "ones")
        throw std::runtime_error("Invalid dataset");

    if (o.block != 128 && o.block != 256 && o.block != 512)
        throw std::runtime_error("Use block 128, 256 or 512");

    if (o.n > size_t(std::numeric_limits<int>::max()) ||
        o.warmup < 0 || o.samples < 1 || o.batch < 1 || o.gap_us < 0)
        throw std::runtime_error("Invalid size/count");

    if (!std::isfinite(o.atol) || !std::isfinite(o.rtol) ||
        o.atol < 0 || o.rtol < 0)
        throw std::runtime_error("Invalid tolerance");

    if (!o.check_only && o.n == 0)
        throw std::runtime_error("N=0 belongs in correctness tests, not this timing study");
...
struct Work {
    ...
    explicit Work(Options a)
        : o(a), x(a.n), y(a.n), z(a.n), p(a.n), q(a.n), result(1),
          hx(a.n), hy(a.n), got(a.op == "reduce" ? 1 : a.n), ref(got.size()) {

        CUDA_OK(cudaGetDeviceProperties(&prop, 0));

        if (o.block > prop.maxThreadsPerBlock || prop.warpSize != 32)
            throw std::runtime_error("Unsupported block/warp size");

        if (o.fixture == "last_nonzero") {
            std::fill(hx.begin(), hx.end(), 0.0f);
            if (o.n) hx[o.n - 1] = 1.0f;

        } else if (o.fixture == "cancellation") {
            if (o.n != 3)
                throw std::runtime_error("Cancellation fixture requires N=3");

            hx[0] = 100000000.0f;
            hx[1] = 1.0f;
            hx[2] = -100000000.0f;

        } else if (o.fixture == "normal") {
            ...
            for (size_t i = 0; i < o.n; ++i) {
                ...
                hx[i] = o.dataset == "ones" ? 1.0f
                      : o.dataset == "dyadic" ? float(int(i % 17) - 8) / 16.0f
            ...
            throw std::runtime_error("Unknown internal fixture");
        }

        if (o.op == "reduce") {
            double s = 0;
            for (float v : hx) s += double(v);
            ref[0] = s;
        } else {
            for (size_t i = 0; i < o.n; ++i)
                ref[i] = o.op == "copy" ? double(hx[i])
                       : o.op == "add" ? double(hx[i]) + double(hy[i])
                       : 2.0 * double(hx[i]) + 1.0;
        }
        ...
        if (o.n) {
            ...
            CUDA_OK(cudaMemcpy(
                x.p, hx.data(), o.n * sizeof(float), cudaMemcpyHostToDevice));

            CUDA_OK(cudaMemcpy(
                y.p, hy.data(), o.n * sizeof(float), cudaMemcpyHostToDevice));
        ...
        if (o.op == "reduce" && o.variant == "cub" && o.n) {
            CUDA_OK(cub::DeviceReduce::Sum(
                nullptr, cub_bytes, x.p, result.p, int(o.n), stream.s));

            CUDA_OK(cudaMalloc(
                &cub_temp, std::max(size_t(1), cub_bytes)));
        }
    }
    ...
    unsigned blocks(size_t n) const {
        return unsigned(std::min(
            (n + o.block - 1) / o.block,
            size_t(prop.multiProcessorCount) * 4));
    }
    ...
    void enqueue() {
        if (o.op != "reduce") {
            if (o.n) {
                ...
                elementwise<<<blocks(o.n), o.block, 0, stream.s>>>(
                    x.p, y.p, z.p, o.n, op);
        ...
        if (o.n == 0) {
            CUDA_OK(cudaMemsetAsync(result.p, 0, sizeof(float), stream.s));
            return;
        }

        if (o.variant == "cub") {
            CUDA_OK(cub::DeviceReduce::Sum(
                cub_temp, cub_bytes, x.p, result.p, int(o.n), stream.s));
        ...
        do {
            ...
            if (o.variant == "pair")
                pair_stage<<<unsigned((count + o.block - 1) / o.block),
                             o.block, 0, stream.s>>>(src, dst, m);
            else if (o.variant == "tree")
                tree_stage<<<unsigned(count), o.block,
                             o.block * sizeof(float), stream.s>>>(src, dst, m);
            else
                warp_stage<<<unsigned(count), o.block, 0, stream.s>>>(
                    src, dst, m);
    ...
    // Completion + host data availability; no correctness judgment.
    void readback() {
    ...
    double validate() {
        ...
        for (size_t i = 0; i < got.size(); ++i) {
            ...
            if (!std::isfinite(got[i]) ||
                e > o.atol + o.rtol * std::abs(ref[i]))
                throw std::runtime_error(
                    "Correctness failed at index " + std::to_string(i) +
...
int main(int argc, char** argv) {
    try {
        ...
        std::cerr
            << "nvtx=disabled "
               "(rebuild with LAB_USE_NVTX for the annotated-trace exercise)\n";
#endif

        if (o.check_only) {

            // Existing normal correctness suite.
            for (size_t n : {
                     size_t(0), size_t(1), size_t(7), size_t(31),
                     size_t(32), size_t(33), size_t(127), size_t(128),
                     size_t(129), size_t(255), size_t(256), size_t(257),
                     size_t(511), size_t(512), size_t(513),
                     size_t(1003), size_t(4099)})
            ...
            // Missing-tail fixture: final valid element carries the whole result.
            {
                Options c = o;
                c.n = 1003;
                c.op = "reduce";
                c.variant = "tree";
                c.fixture = "last_nonzero";
                ...
                std::cout
                    << "PASS last-valid-element fixture"
                    << " N=1003"
                    << " variant=tree"
                    << " block=" << o.block
                    << '\n';
            ...
            // Numerical-order observation: do not force through validate().
            {
                Options c = o;
                c.n = 3;
                c.op = "reduce";
                c.variant = "pair";
                c.fixture = "cancellation";

                Work w(c);
                w.enqueue();
                w.readback();

                double device = double(w.got[0]);
                double reference = w.ref[0];
                double error = std::abs(device - reference);
                double tolerance = c.atol + c.rtol * std::abs(reference);
                bool within_tolerance =
                    std::isfinite(w.got[0]) && error <= tolerance;

                std::cout
                    << std::setprecision(10)
                    << "OBS cancellation fixture"
                    << " N=3"
                    << " variant=pair"
                    << " block=" << o.block
                    << " device=" << device
            ...
            std::cout
                << "PASS boundary suite at block=" << o.block
                << "; this is not a proof for arbitrary inputs\n";
        ...
        { Range range("warmup");
          for (int k = 0; k < o.warmup; ++k) w.enqueue(); }

        // One untimed correctness call, even if warmup == 0.
        w.enqueue();
        w.validate();

        std::cout
            << "sample,n,op,variant,block,warmup,batch,gap_us,dataset,"
               "event_span_ms,host_submit_ms,host_complete_ms,"
               "event_ms_per_op,max_abs_error\n";
        ...
        for (int sample = 0; sample < o.samples; ++sample) {
...

            {
                ...
                for (int k = 0; k < o.batch; ++k) {
                    if (o.gap_us)
                        std::this_thread::sleep_for(
                            std::chrono::microseconds(o.gap_us));
                ...
                submit = std::chrono::duration<double, std::milli>(
                    submit1 - submit0).count();

                complete = std::chrono::duration<double, std::milli>(
                    t1 - t0).count();
            }

            double error = w.validate();

            std::cout
                << sample << ','
                << o.n << ','
                << o.op << ','
                << (o.op == "reduce" ? o.variant : "na") << ','
                << o.block << ','
                << o.warmup << ','
                << o.batch << ','
                << o.gap_us << ','
